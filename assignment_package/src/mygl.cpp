...
void MyGL::RenderScene()
{
    ...
    // Do a pre-pass that bounces the photons through the scene
    // and stores them.
    if(integratorType == PHOTON_MAPPING)
    {
        ...
        // copying the photons into the tree so we can delete
        // the array of photons created here.
    ...
    for(int X = 0; X < nTiles.x; X++)
    {
        for(int Y = 0; Y < nTiles.y; Y++)
        {
            ...
            case NAIVE_LIGHTING:
                ...
                break;
            case PHOTON_MAPPING:
                rt = new PhotonMapper(tileBounds, &scene, sampler->Clone(seed), recursionLimit);
                break;
            }
...
void MyGL::slot_SetPhotonCount(int p)
{
    numPhotons = p;
}

void MyGL::slot_SetSearchRadius(double r)
{
